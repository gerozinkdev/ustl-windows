...
{
    int rv = -1;//::ioctl (m_fd, request, argument);
    if (rv < 0)
...
{
    int rv = -1;//::fcntl (m_fd, request, argument);
    if (rv < 0)
	set_and_throw (failbit, rname);
    return (rv);
}
...
{
    void* result = 0; //::mmap (NULL, n, PROT_READ | PROT_WRITE, MAP_SHARED, m_fd, offset);
