...
void TestString (void)
{
    ...
    for (i = 0; i < s3.length(); ++ i)
	s3[i] = s3[i];
    cout.format ("%s\ns3.size() = %lu, max_size() = ", s3.c_str(), s3.size());
    if (s3.max_size() == SIZE_MAX - 1)
	cout << "(SIZE_MAX/elsize)-1";
    else
	cout << s3.max_size();
    cout.format (", capacity() = %lu\n", s3.capacity());
    ...
    s1 = c_TestString1;
    cout.format ("8 found at %lu\n", s1.find ('8'));
    cout.format ("9 found at %lu\n", s1.find ("9"));
    cout.format ("7 rfound at %lu\n", s1.rfind ('7'));
    cout.format ("7 rfound again at %lu\n", s1.rfind ('7', s1.rfind ('7') - 1));
    cout.format ("67 rfound at %lu\n", s1.rfind ("67"));
    if (s1.rfind("X") == string::npos)
	cout << "X was not rfound\n";
    else
	cout.format ("X rfound at %lu\n", s1.rfind ("X"));
    uoff_t poundfound = s1.find ("#");
    if (poundfound != string::npos)
	cout.format ("# found at %lu\n", poundfound);
    cout.format ("[456] found at %lu\n", s1.find_first_of ("456"));
    cout.format ("[456] last found at %lu\n", s1.find_last_of ("456"));

    s2.clear();
    assert (!*s2.end());
    if (s2.empty())
	cout.format ("s2 is empty [%s], capacity %lu bytes\n", s2.c_str(), s2.capacity());

    s2.format ("<const] %d, %s, 0x%08X", 42, "[rfile>", 0xDEADBEEF);
    cout.format ("<%lu bytes of %lu> Format '%s'\n", s2.length(), s2.capacity(), s2.c_str());
